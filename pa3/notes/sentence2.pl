...
majorOf(X,Y) :- occupies(X,Y).


%%%%%%%%%%%%% exists %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

add_rule(Students,occupies,M,P) :- major(M), poster(P), assertz(occupies(M,P)).
add_rule(Students,occupies,G,P) :- genre(G), poster(P), assertz(occupies(M,P)).
add_rule(Students,occupies,Q,P) :- pizza(Q), poster(P), assertz(occupies(M,P)).
add_rule(Students,occupies,C,P) :- club(C),  poster(P), assertz(occupies(M,P)).
